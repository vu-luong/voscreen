...
crawler_type = input('Input 1 to crawl all, Input 2 to crawl each mode: ')

if crawler_type == '2':
    gn = input("Input mode in {}: ".format(modes))
    current_modes = [gn]
else:
...
while True:
    iter = iter + 1
    if iter >= len(current_modes):
        break

    group_name = current_modes[iter]

    print('Crawling {}'.format(group_name))
    ...
    while True:
        try:
            ...
            if 'Web server is down' in str(ques_res_json):
